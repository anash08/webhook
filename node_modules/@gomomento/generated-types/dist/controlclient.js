...
(function (control_client) {
    ...
    class _ListIndexesResponse extends pb_1.Message {
        constructor(data) {
            ...
            if (!Array.isArray(data) && typeof data == "object") {
                if ("index_names" in data && data.index_names != undefined) {
                    this.index_names = data.index_names;
                }
            }
        }
        get index_names() {
        ...
        set index_names(value) {
            pb_1.Message.setField(this, 1, value);
        }
        static fromObject(data) {
            const message = new _ListIndexesResponse({});
            if (data.index_names != null) {
                message.index_names = data.index_names;
            }
            return message;
        }
        toObject() {
            const data = {};
            if (this.index_names != null) {
                data.index_names = this.index_names;
            }
            return data;
        }
        serialize(w) {
            const writer = w || new pb_1.BinaryWriter();
            if (this.index_names.length)
                writer.writeRepeatedString(1, this.index_names);
            if (!w)
                return writer.getResultBuffer();
        }
        static deserialize(bytes) {
